...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backfill the last year of commits into ClickHouse.")
    parser.add_argument("--days", type=int, default=365, help="How many days back to fetch commits.")
...
def _delegate_to_container(argv: list[str]) -> int:
    ...
    with Path(__file__).open("r", encoding="utf-8") as script_source:
        result = subprocess.run(command, stdin=script_source, check=False)
