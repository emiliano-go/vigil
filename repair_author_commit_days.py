...
def _run_inside_container(argv: list[str]) -> int:
    script_path = Path("/app/scripts/repair_author_commit_days.py")
    if script_path.exists():
        sys.argv = [str(script_path), *argv]
        runpy.run_path(str(script_path), run_name="__main__")
        return 0
    raise SystemExit("repair_author_commit_days.py is only available inside the app container at /app/scripts")
...
def _delegate_to_container(argv: list[str]) -> int:
    ...
    command = [
        ...
        "run",
        "python",
        "/app/scripts/repair_author_commit_days.py",
        *argv,
    ]
