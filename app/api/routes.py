...
def daily_author_stats(days: int = Query(default=7, ge=1, le=365), author_login: str | None = None):
    ...
    totals = _query_dicts(
        f"SELECT toDate(committed_at, '{tz}') AS period, uniqExact(sha) AS total FROM commits {where} GROUP BY period ORDER BY period DESC",
        params,
    )
    by_author = _query_dicts(
        f"SELECT toDate(committed_at, '{tz}') AS period, {settings.canonical_author_login_expr()} AS author_login, uniqExact(sha) AS total FROM commits {where} GROUP BY period, author_login ORDER BY period DESC, author_login",
        params,
    )
